 ...
 */
package quaternary.botaniatweaks.compat.botania.tile.render;
...
import quaternary.botaniatweaks.compat.botania.tile.TileCompressedTinyPotato;
...
public class RenderTileCompressedTinyPotato extends TileEntitySpecialRenderer<TileCompressedTinyPotato> {
	...
	public void render(TileCompressedTinyPotato potato, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		...
		int compressionLevel = potato.compressionLevel;
		ModelCompressedTinyPotato model = models[compressionLevel - 1];
		...
		float angle = potato.getWorld().getBlockState(potato.getPos()).getValue(BlockCompressedTinyPotato.FACING).getHorizontalAngle();
		float rotY = angle;
		...
		EnumFacing potatoFacing = potato.getWorld().getBlockState(potato.getPos()).getValue(BlockCompressedTinyPotato.FACING);
		...
		if(!name.isEmpty() && pos != null && pos.getBlockPos() != null && potato.getPos().equals(pos.getBlockPos())) {
			...
			float scale = (float) MathUtil.rangeRemap(compressionLevel, 0, 8, 1, 4); //added
			GlStateManager.translate(0, scale / 3, 0); //added
			...
			GlStateManager.scale(scale, scale, scale); //added
